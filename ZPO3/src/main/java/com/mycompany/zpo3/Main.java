...
public class Main {
    ...
    public static List<Pizza> iLikeMeat(List<Pizza> pizzaList){
        List <Pizza> meatPizzas=new ArrayList<>((Collection<? extends Pizza>) pizzaList.stream()
                .filter(p->p.getIngredients()
                    .stream()
                    .allMatch(i->i.isMeat()))
                .collect(groupingBy(p->p.getIngredients().stream().filter(i->i.isMeat()).count())));
        return meatPizzas;
    }
    ...
    public static String formatedMenu(List<Pizza> pizzaList){
        return pizzaList.stream()
                .map(p->String.format(" %s : %s- %d pln",
                        p.toString(),
                        p.getIngredients().toString(),
